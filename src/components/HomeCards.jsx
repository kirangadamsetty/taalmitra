...
function HomeCards({data}) {
    const { handleFilteredData,setSelectedPlayList} = useContext(SongsContext)
...
<p className  = "detail-info detail-info-head">{data[0].detailInfo}</p>
    ...
    >
      ...
      {data.slice(1).map((li) => (
        <>
        <SwiperSlide key={li.id} style = {{cursor:"pointer"}} onClick  ={()=>{setSelectedPlayList(li);handleFilteredData(li);navigate("/songsInfo")}} className="home-card-container">
